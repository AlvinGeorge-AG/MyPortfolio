    ...
    const fetchData = async () => {
      try {
        ...
        const extraProjs = [
          {
            heading: "Odin CLI",
            note: "A developer CLI toolkit for Linux written in Go using Cobra. It abstracts painful commands (like freeing ports, analyzing disk usage, managing files) into simple subcommands.",
            img: "/images/odin.png",
            githubLink: "https://github.com/AlvinGeorge-AG/odin"
          },
          {
            heading: "AI-Enhanced DevOps Assistant",
            note: "An AI-powered SRE agent built with FastAPI and Python. It monitors infrastructure metrics, listens to Prometheus/Alertmanager webhooks, analyzes logs, and takes autonomous scaling/healing actions using LLMs.",
            img: "/images/aidevops.png",
            githubLink: "https://github.com/AlvinGeorge-AG/AI-Enhanced-DevOps-Assistant"
          },
          {
            heading: "LMonitor",
            note: "A lightweight Linux system monitoring agent that collects system metrics (CPU, RAM, Disks, Network) and pushes them over WebSockets to a live dashboard featuring beautiful interactive charts.",
            img: "/images/lmonitor.png",
            githubLink: "https://github.com/AlvinGeorge-AG/LMonitor"
          },
          {
            heading: "TEGO",
            note: "A local services platform designed with Express.js, Prisma ORM, and PostgreSQL. It features full JWT-based authentication, service discovery, booking workflows, and an intuitive user dashboard.",
            img: "/images/tego.png",
            githubLink: "https://github.com/AlvinGeorge-AG/TEGO"
          }
        ];

        const mergedLangs = [...langResponse.data];
        extraLangs.forEach(extra => {
          if (!mergedLangs.some(l => l.name.toLowerCase() === extra.name.toLowerCase())) {
            mergedLangs.push(extra);
          }
        });

        const mergedProjs = [...projResponse.data];
        extraProjs.forEach(extra => {
          if (!mergedProjs.some(p => p.heading.toLowerCase() === extra.heading.toLowerCase())) {
            mergedProjs.push(extra);
          }
        });
      ...
      <div className="animated-bg-content">
        ...
        >
          <div className="flex flex-col h-full overflow-y-auto">
            {/* Close Button (Mobile) */}
            <div onClick={() => setMenuOpen(false)} className="desktop-hidden absolute top-4 right-4 text-gray-400 hover:text-white cursor-pointer z-50 p-2">
            ...
            <div className="flex flex-col items-center pt-12 pb-8 px-6 text-center border-b border-zinc-800">
              ...
              <span className="text-xs text-gray-400 mt-2 uppercase tracking-widest">ML & DevOps Enthusiast</span>
            </div>

            <nav className="flex-1 py-8">
              <ul className="flex flex-col space-y-2">
                {["About Me", "Tech Skills", "My Projects", "Certifications", "Experience", "Resume", "Contact Me"].map((text, idx) => {
                  const ids = ["#1", "#3", "#2", "#certs", "#6", "#4", "#5"];
                  return (
                    <li key={idx}>
                      <a
                        href={ids[idx]}
        ...
        <main className="md:ml-72 transition-all duration-300">
            ...
            <div className="flex flex-col md:flex-row gap-12 items-center mb-16">
              ...
              <div className="w-full md:w-7/12">
                ...
                <p className="text-gray-300 leading-relaxed text-lg">
                  Hi, I'm <span className="font-bold text-white">Alvin George</span>, a B.Tech Computer Science student at <span className="text-blue-400">Model Engineering College</span>, Cochin. I specialize in building scalable backend systems using FastAPI and Spring Boot, and I'm actively pursuing ML & DevOps Engineering. With hands-on experience in CI/CD pipelines (Jenkins, Docker, AWS), IaC stacks (Terraform), and observability (Prometheus/Grafana), I love turning complex infrastructure problems into automated, self-healing systems.
                </p>
              </div>
            </div>
          ...
          <section id="certs" className="py-24 px-6 md:px-12 max-w-6xl mx-auto border-t border-zinc-800">
            <div className="mb-16 text-center">
              <h2 className="text-3xl font-bold uppercase text-white tracking-wider">Licenses & Certifications</h2>
              <div className="h-1 w-24 bg-gradient-to-r from-blue-500 to-indigo-500 mx-auto mt-4 mb-4 shadow-[0_0_15px_rgba(59,130,246,0.6)]"></div>
            </div>
              ...
              {[
                {
                  ...
                  skills: "API Testing, Api handling, and Postman API",
                  icon: "fa-rocket",
                  color: "from-orange-500/20 to-red-500/5"
                },
                {
                  title: "Infosys certified JavaScript Course",
                  issuer: "Infosys Springboard",
                  date: "Issued Jun 2025",
                  skills: "JavaScript, JSON, DOM Manipulation",
                  icon: "fa-code",
                ...
                },
                {
                  ...
                  skills: "Computer Vision, OpenCV, ML & Python",
                  icon: "fa-eye",
                ...
                <div
                  key={idx}
                  className="relative bg-zinc-900/60 backdrop-blur-sm border border-zinc-800 p-8 rounded-2xl shadow-xl transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_10px_30px_rgba(0,0,0,0.5)] hover:border-blue-500/30 flex flex-col justify-between group overflow-hidden"
                >
                  <div className={`absolute -right-16 -top-16 w-36 h-36 rounded-full bg-gradient-to-br ${cert.color} blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500`}></div>

                  <div>
                    <div className="flex items-center justify-between mb-6">
                      <span className="text-xs font-mono text-blue-400 bg-blue-950/40 border border-blue-900/50 px-3 py-1 rounded-full uppercase tracking-wider">
                        {cert.issuer}
                      </span>
                      <i className={`fa ${cert.icon} text-2xl text-gray-500 group-hover:text-blue-400 transition-colors`}></i>
                    </div>

                    <h4 className="text-xl font-bold text-white group-hover:text-blue-400 transition-colors duration-300 mb-2 leading-snug">
                      {cert.title}
                    </h4>
                    <p className="text-gray-400 text-sm mb-4 font-mono">{cert.date}</p>
                  </div>

                  <div className="border-t border-zinc-800/80 pt-4 mt-4">
                    <span className="text-[11px] uppercase tracking-wider text-gray-500 block mb-1">Inferred Skills:</span>
                    <span className="text-xs text-gray-300 font-mono">{cert.skills}</span>
                  </div>
                </div>
              ))}
            </div>
